...
void Solve::imageprocess(){
	...
	// filter the image in rgb space, and finally save the image in binary1
	...
	// the image shown by cvShowImage function is flipped and we need to 
}
void Solve::work(){
        
    //init
    memset(processing,0,sizeof(processing));
    //imageprocess function to do some processing like Morphology Transformation 
    imageprocess();

    //calc
    p=5; 
    computer_stereo_vision_for_calculation(0,cos_threshold,step,count_threshold);
	
	// in this function, we calculate and detect the finger tips 
...
void Solve::computer_stereo_vision_for_calculation(int start,double cos_threshold,int step,int count_threshold){
